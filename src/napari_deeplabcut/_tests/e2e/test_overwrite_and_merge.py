...
def test_config_layer_then_h5_then_config_again_uses_plugin_save_checks(
    ...
    assert not keypoint_controls.layer_manager.is_empty_points_layer(config_layer)

    # The manager must reconcile/promote this before plugin save routing.
    assert keypoint_controls.layer_manager.prepare_points_layer_for_plugin_save(config_layer)
    assert get_layer_role_from_metadata(config_layer.metadata) is LayerRole.DLC_ANNOTATION
    assert (
        keypoint_controls.layer_manager.save_behavior_for_points_layer(config_layer) is LayerSaveBehavior.PLUGIN_MANAGED
    )
    ...
    assert get_layer_role_from_metadata(points.metadata) is LayerRole.DLC_ANNOTATION
    assert keypoint_controls.layer_manager.save_behavior_for_points_layer(points) is LayerSaveBehavior.PLUGIN_MANAGED

    # Add config.yaml again. This should not create an invalid generic-save path.
    viewer.open(str(config_path), plugin="napari-deeplabcut")
    qtbot.wait(300)

    # The authoritative H5-loaded annotation layer should remain plugin-managed.
    assert keypoint_controls.layer_manager.prepare_points_layer_for_plugin_save(points)
    assert get_layer_role_from_metadata(points.metadata) is LayerRole.DLC_ANNOTATION
    assert keypoint_controls.layer_manager.save_behavior_for_points_layer(points) is LayerSaveBehavior.PLUGIN_MANAGED
