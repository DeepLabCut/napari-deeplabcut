...
def test_projectless_folder_save_refuses_unresolved_outside_paths(
    ...
    """
    Contract: project-association save must refuse if any path cannot be
    rewritten to a canonical DLC dataset row key.

    DLC annotation H5 row keys must be:
        labeled-data/<dataset>/<image>

    Unrelated absolute paths must never be written into CollectedData_*.h5.
    """
    overwrite_confirm.forbid()
