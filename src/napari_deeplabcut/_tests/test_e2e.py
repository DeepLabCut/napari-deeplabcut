...
def _rgba_from_metadata_cycle(points_layer: Points, *, prop: str, value) -> np.ndarray:
    """
    Resolve expected RGBA from napari-deeplabcut metadata face_color_cycles.

    Works even if `value` is a numpy scalar; keys in metadata are stored as strings.
    """
    md = points_layer.metadata or {}
    cycles = (md.get("face_color_cycles") or {}).get(prop) or {}
    key = str(value)
    assert key in cycles, f"Missing key {key!r} in metadata face_color_cycles[{prop!r}]. keys={list(cycles)[:10]}"
    return np.asarray(cycles[key], dtype=float)
...
def _snapshot_color_metadata(layer: Points) -> dict[str, Any]:
    md = layer.metadata or {}
    return {
        "colormap_name": md.get("colormap_name") or md.get("colormap"),
        "face_color_cycles": deepcopy(md.get("face_color_cycles")),
        # optional: include header fingerprint to make debugging easier
        "header": deepcopy(md.get("header")),
    }


def _assert_cycles_equal(a: dict | None, b: dict | None):
    assert isinstance(a, dict) and a, f"face_color_cycles missing/empty: {a!r}"
    assert isinstance(b, dict) and b, f"face_color_cycles missing/empty: {b!r}"

    # Compare only the keys we rely on (label/id) to avoid unrelated diffs
    for k in ("label", "id"):
        if k in a or k in b:
            assert k in a and k in b, f"Cycle '{k}' present in one but not the other. a={a.keys()}, b={b.keys()}"
            # Compare mapping keys and RGBA arrays
            assert set(a[k].keys()) == set(b[k].keys()), f"Cycle keys differ for '{k}'"
            for name in a[k].keys():
                av = np.asarray(a[k][name], dtype=float)
                bv = np.asarray(b[k][name], dtype=float)
                assert np.allclose(av, bv, atol=1e-6), f"RGBA mismatch for cycle '{k}' key '{name}'"


@pytest.mark.usefixtures("qtbot")
def test_colormap_metadata_roundtrip_config_then_folder_then_save(
    make_napari_viewer, qtbot, tmp_path, overwrite_confirm
):
    """
    E2E regression for: config-first workflow losing metadata like colormap on save.

    This asserts a true round-trip:
      - snapshot pre-save colormap_name + face_color_cycles
      - save
      - reload from disk
      - compare snapshots
    """
    ...
    # Make the config colormap distinctive so we can see if it falls back.
    import yaml
    ...
    # ---- Session 1: open config, open folder, annotate, save ----
    viewer = make_napari_viewer()
    from napari_deeplabcut._widgets import KeypointControls
    ...
    # Ensure we are testing the GT layer from the expected file (important if placeholder exists)
    if "source_h5" in (points.metadata or {}):
        assert Path(points.metadata["source_h5"]).name == h5_path.name

    store = controls._stores.get(points)
    assert store is not None

    # Sanity: config colormap should have been applied pre-save
    pre = _snapshot_color_metadata(points)
    assert pre["colormap_name"] == "magma", f"Pre-save colormap mismatch: {pre['colormap_name']!r}"
    assert pre["face_color_cycles"], "Pre-save face_color_cycles missing (cannot test round-trip)."

    # Add an annotation and save
    _set_or_add_bodypart_xy(points, store, "bodypart2", x=44.0, y=33.0)
    viewer.layers.selection.active = points
    viewer.layers.save("__dlc__.h5", selected=True, plugin="napari-deeplabcut")
    qtbot.wait(250)

    # ---- Session 2: reload from disk and compare ----
    viewer2 = make_napari_viewer()
    from napari_deeplabcut._widgets import KeypointControls as KC2

    controls2 = KC2(viewer2)
    viewer2.window.add_dock_widget(controls2, name="Keypoint controls", area="right")

    viewer2.open(str(h5_path), plugin="napari-deeplabcut")
    qtbot.waitUntil(lambda: any(isinstance(ly, Points) for ly in viewer2.layers), timeout=10_000)
    qtbot.wait(200)

    pts2 = next(ly for ly in viewer2.layers if isinstance(ly, Points))
    post = _snapshot_color_metadata(pts2)

    # Primary assertion: colormap_name round-trips
    assert post["colormap_name"] == pre["colormap_name"], (
        "colormap_name did not round-trip.\n"
        f"  pre:  {pre['colormap_name']!r}\n"
        f"  post: {post['colormap_name']!r}\n"
        f"  post keys: {sorted((pts2.metadata or {}).keys())}"
    )

    # Secondary assertion: cycles round-trip (this is the real user-visible behavior)
    _assert_cycles_equal(pre["face_color_cycles"], post["face_color_cycles"])
