...
This release brings a nearly-complete rewrite of the plugin, keeping a familiar UI and workflow but adding many new **safety improvements**, **user messages/debugging information**, and **quality-of-life improvements**.

> [!NOTE]
> As much of the basic workflow has been modified and updated, experienced users may be able to simplify how they load and save data. Expect a slight adjustment period as you get used to the new workflow, but we hope you will appreciate the improvements and new features!
...
- **New "progress estimate" widget**, showing the estimated % frames labeled in the current folder.
  - Note: the estimate should be considered a relative progress metric, rather than a requirement to reach 100% on every folder, as the estimate cannot account for non-visible keypoints.
...
- **Fixed & improved trails**:
  ...
  - Basic visualization settings are persisted across sessions
...
- **Fixed and improved trajectory plot**:
  - Colors are updated to match individuals/bodyparts in the viewer
  - Selecting points automatically shows only the trajectories for those points, rather than all trajectories
...
- **Drag & drop loading is fixed!**
  ...
  - > [!IMPORTANT]
  Users no longer need to always add the config.yaml to the viewer. The plugin now checks parent folders as per the canonical DLC project structure, and will automatically find and load the config.yaml if it is present in a parent folder.
...
- **Any save that would overwrite data now shows a confirmation dialog**
  - Can be disabled with the "Warn on overwrite" option in the Keypoint Controls, if one is labeling from sratch and requires faster saves
...
- Improved data loading checks and remapping of frames
  - Moving data between folders or machines should cause less issues with frame remapping, and the plugin should be able to recover from more types of data loading issues without crashing
...
Expect a focus on fixes and improvements shortly after this major updated, followed by more features and improvements, such a automated point tracking for faster and more robust labeling.
